...
/// Polkadot-JS API Ledger Connection
#[wasm_bindgen]
pub struct SBTPolkadotJsLedger(Api);

#[wasm_bindgen]
impl SBTPolkadotJsLedger {
    /// Builds a new [`SBTPolkadotJsLedger`] from its JS [`Api`].
    #[inline]
    #[wasm_bindgen(constructor)]
    pub fn new(api: Api) -> Self {
        console_error_panic_hook::set_once();
...
impl ledger::Connection for SBTPolkadotJsLedger {
    type Error = LedgerError;
}

impl ledger::Read<SyncData<config::Config>> for SBTPolkadotJsLedger {
    type Checkpoint = utxo::Checkpoint;
    ...
    fn read<'s>(
        ...
        Box::pin(async {
            Ok(
                from_js::<RawPullResponse>(self.0.sbt_pull(borrow_js(checkpoint)).await)
                    .try_into()
                    .expect("Conversion is not allowed to fail."),
            )
        })
...
impl ledger::Write<Vec<config::TransferPost>> for SBTPolkadotJsLedger {
    type Response = String;
...
/// Wallet with Polkadot-JS API Connection
#[wasm_bindgen]
pub struct SBTWallet(Rc<RefCell<SBTWalletType>>);
...
impl SBTWallet {
    ...
    #[inline]
    #[wasm_bindgen(constructor)]
    pub fn new(ledger: SBTPolkadotJsLedger, signer: Signer) -> Self {
        Self(Rc::new(RefCell::new(SBTWalletType::new(ledger, signer.0))))
    }
    ...
    where
        T: Serialize,
        E: Debug,
        F: 'static + for<'w> FnOnce(&'w mut SBTWalletType) -> LocalBoxFutureResult<'w, T, E>,
    {
        let this = self.0.clone();
        let response = future_to_promise(async move {
            f(&mut this.borrow_mut())
